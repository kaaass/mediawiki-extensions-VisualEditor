...
class ApiVisualEditor extends ApiBase
{
    ...
    private function requestJSONAPI($action, $params, $reqheaders = [])
    {
        global $wgVersion;
        global $wgScriptPath;
        global $wgServer;
        ...
        } else if ($response['code'] != 200) { // error null, code not 200
    ...
    {
        $data = $this->requestJSONAPI('expandtemplates', ['text' => $html, 'redirects' => false]);
        isset($data['expandtemplates']['*']) or $this->dieUsage('docserver-http-error', 'parse error');
        /* preg_match("/<body[^>]*?>(.*\s*?)<\/body>/is", $data['expandtemplates']['*'], $match);
         return $match[1];*/
    ...
    {
        $data = $this->requestJSONAPI('parse', ['text' => $wikitext, 'disablepp' => 'true']);
        isset($data['parse']['text']['*']) or $this->dieUsage('docserver-http-error');
        ...
        switch ($params['paction']) {
            ...
            case 'metadata':
                ...
                if ($title->exists()) {
                    ...
                    // If requested, request HTML from API
                    if ($params['paction'] === 'parse') {
                        $data = $this->requestJSONAPI('parse', ['oldid' => $oldid, 'redirects' => false, 'disablepp' => 'true']);
                        isset($data['parse']['text']['*']) or $this->dieUsage('Error contacting the document server', 'docserver');
                        $content = substr($data['parse']['text']['*'], 0, strpos($data['parse']['text']['*'], "\n\n<!-- \nNewPP limit report"));
                    }
